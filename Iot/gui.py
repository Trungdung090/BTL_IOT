...
def save_log(people_count, image_name):
    if people_count >= 1:   # Chỉ lưu nếu phát hiện ít nhất 1 người
        ...
        # Thêm hoặc cập nhật dữ liệu nếu đã tồn tại
        db_queue.put(("INSERT INTO logs (time, people_count, image_name) VALUES (?, ?, ?) "
                      "ON CONFLICT(time) DO UPDATE SET people_count = excluded.people_count, image_name = excluded.image_name",
                      (time_now, people_count, image_name)))
...
def get_hourly_density():
    ...
    cursor.execute("""
        SELECT strftime('%H', time) as hour, SUM(people_count) as total_people 
        FROM logs 
    ...
    data = [{"hour": row[0], "people_count": row[1]} for row in cursor.fetchall()]
    conn.close()
    return jsonify(data)
...
def get_logs():
    conn = sqlite3.connect("access_control.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, time, people_count, image_name FROM logs ORDER BY id DESC LIMIT 20")  # Lấy 20 bản ghi mới nhất
